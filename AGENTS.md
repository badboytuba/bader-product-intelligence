...
Release `16.0.1.3.0` is under development on `feature/bpi-technical-description`. It separates a 45–70 word optimized commercial summary from the new sanitized `bpi_technical_description` HTML field, generated at 350–650 words. The backend reuses the allowlisted rich editor with a compact commercial surface and a larger technical surface. Public technical content belongs after `product_detail_main` and before FAQs. Existing catalog descriptions must never be truncated or migrated automatically; the split is applied only after an operator generates/edits and saves the new content.
